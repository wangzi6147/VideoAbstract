...
bool CDataMySql::CreateCombineSegsTable(CString CombineSegsTableName)
{
	...
	m_strSql.Format("create table %s(nOldPara int,segID int not null, nX smallint(3) not null,\
		nY smallint(3) not null,nWidth smallint(3) not null,nHeight smallint(3) not null,origFrame int not Null,PicID int not Null)", CombineSegsTableName);
	if (mysql_real_query(&m_mysql, (char*)(LPCTSTR)m_strSql, (UINT)m_strSql.GetLength()) != 0)
	{
		print_error(&m_mysql, "error message");
		return false;
...
{
	CString m_strSql;///<mysql查询语句
	m_strSql.Format("insert into %s(nOldPara, segID, nX, nY, nWidth, nHeight,origFrame,PicID)\
		values(\'%d\',\'%d\',\'%d\',\'%d\',\'%d\',\'%d\',\'%d\',\'%d\')",\
		CombineSegsTableName, traceTab.nOldPara, traceTab.segID, traceTab.nX, traceTab.nY, traceTab.nWidth, traceTab.nHeight, traceTab.origFrame, traceTab.PicID);
	if (mysql_real_query(&m_mysql, (char*)(LPCTSTR)m_strSql, (UINT)m_strSql.GetLength()) != 0)
	{
		print_error(&m_mysql, "error message");
		return false;
...
///@retval BOOL 返回操作是否成功
///@post NULL
bool CDataMySql::FindROIFromCombineSegsTable(vector <OrigTraceTable> *myVector, CString tableName)
{
	CString m_strSql;
	MYSQL_RES *m_result;
	MYSQL_ROW m_sqlRow;
	m_strSql.Format("select nX,nY,nWidth,nHeight,PicID from %s ", tableName);
	if (mysql_real_query(&m_mysql, (char*)(LPCTSTR)m_strSql, (UINT)m_strSql.GetLength()) != 0)
	{
		m_result = NULL;///<出错
		print_error(&m_mysql, "error message");
	...
	else
	{
		...
		{
			m_sqlRow = mysql_fetch_row(m_result);///<获得所有结果字符串
			OrigTraceTable myValue = { 0, 0, 0, 0, 0, 0, 0, 0, 0 };
			myValue.nX = atoi(m_sqlRow[0]);
			myValue.nY = atoi(m_sqlRow[1]);
			myValue.nWidth = atoi(m_sqlRow[2]);
			myValue.nHeight = atoi(m_sqlRow[3]);
			//myValue.origFrame = atoi(m_sqlRow[4]);
			myValue.PicID = atoi(m_sqlRow[4]);
...
bool CDataMySql::GetObjectInfoFromObjectTable(vector<objectInfo> *objDetectedInfos,CString ObjectTableName)
{
	...
	MYSQL_RES *m_result;
	MYSQL_ROW m_sqlRow;
	m_strSql.Format("select nOldPara,nX,nY,nWidth,nHeight,origFrame from %s",ObjectTableName);//此处若需要在子对话框中显示第(startframe+endframe)/2帧时,nX,nY,nWidth,nHeight,origFrame就无效了
	if (mysql_real_query(&m_mysql, (char*)(LPCTSTR)m_strSql, (UINT)m_strSql.GetLength()) != 0)
	{
		m_result = NULL;///<出错
		print_error(&m_mysql, "error message");
...
bool CDataMySql::GetObjectOriMidFrameFromVideoFgTable(vector<objectInfo> *objDetectedInfos,CString FGTraceTable)
{
	CString m_strSql;
	MYSQL_RES *m_result;
	MYSQL_ROW m_sqlRow;

	for (int i=0;i<objDetectedInfos->size();i++)
	{
		m_strSql.Format("select segID,nWidth,nHeight from %s where nOldPara =\'%d\'",FGTraceTable,objDetectedInfos->at(i).nOldPara);
		if (mysql_real_query(&m_mysql, (char*)(LPCTSTR)m_strSql, (UINT)m_strSql.GetLength()) != 0)
		{
			m_result = NULL;///<出错
			print_error(&m_mysql, "error message");
		...
		else
		{
			m_result = mysql_store_result(&m_mysql);///<保存查询到的数据到m_result
			m_sqlRow=mysql_fetch_row(m_result);
			int maxareaID=atoi(m_sqlRow[0]);//记录第几条记录的矩形面积最大
			int maxarea=atoi(m_sqlRow[1])*atoi(m_sqlRow[2]);
			for (int j=1;j<m_result->row_count;j++)
			{
				m_sqlRow=mysql_fetch_row(m_result);///<获得所有结果字符串
				if (atoi(m_sqlRow[1])*atoi(m_sqlRow[2])>maxarea)
				{
					maxareaID=atoi(m_sqlRow[0]);//记录第几条记录的矩形面积最大
					maxarea=atoi(m_sqlRow[1])*atoi(m_sqlRow[2]);
				}
			}

			m_sqlRow=NULL;
			for (int j=0;j<maxareaID;j++)
			{
				m_sqlRow=mysql_fetch_row(m_result);///<获得所有结果字符串
			}


			objDetectedInfos->at(i).MidSegID=maxareaID;
			if (m_result != NULL)mysql_free_result(m_result);///<释放结果资源
		}
	}
	...
	for (int i=0;i<objDetectedInfos->size();i++)
	{
		m_strSql.Format("select origFrame,nX,nY,nWidth,nHeight from %s where nOldPara =\'%d\' and segID = \'%d\' ", FGTraceTable, objDetectedInfos->at(i).nOldPara,objDetectedInfos->at(i).MidSegID);
		if (mysql_real_query(&m_mysql, (char*)(LPCTSTR)m_strSql, (UINT)m_strSql.GetLength()) != 0)
		{
			m_result = NULL;///<出错
			print_error(&m_mysql, "error message");
		...
		else
		{
			m_result = mysql_store_result(&m_mysql);///<保存查询到的数据到m_result
			m_sqlRow = mysql_fetch_row(m_result);///<获得所有结果字符串
			objDetectedInfos->at(i).MidFrameID=atoi(m_sqlRow[0]);
			objDetectedInfos->at(i).roi.x		= atoi(m_sqlRow[1]);
			objDetectedInfos->at(i).roi.y		= atoi(m_sqlRow[2]);
			objDetectedInfos->at(i).roi.width	= atoi(m_sqlRow[3]);
			objDetectedInfos->at(i).roi.height	= atoi(m_sqlRow[4]);
			if (m_result != NULL)mysql_free_result(m_result);///<释放结果资源
		}
	}
